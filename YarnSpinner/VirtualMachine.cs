...
namespace Yarn
{
    ...
    internal class VirtualMachine
    {
        ...
        public OpCode CurrentOpCode
        {
            get
            {
                if (state.programCounter < currentNode.Instructions.Count && state.programCounter > 0)
                    return currentNode.Instructions[state.programCounter].Opcode;
                else
                    return OpCode.Stop;
            }
        }
        ...
        internal void Continue()
        {
            ...
            while (executionState == ExecutionState.Running)
            {
                ...
                //FastForwardOptionExecution();
            }
        }

        internal void ExecuteInstruction(Instruction instr)
        {
        ...
        internal void FastForwardOptionExecution()
        {
            Instruction currentInstruction = currentNode.Instructions[state.programCounter];
            while (currentInstruction.Opcode == OpCode.AddOption)
            {
                ExecuteInstruction(currentInstruction);
                currentInstruction = currentNode.Instructions[state.programCounter];
            }

            if (currentInstruction.Opcode == OpCode.ShowOptions)
                ExecuteInstruction(currentInstruction);
        }
    }
}
